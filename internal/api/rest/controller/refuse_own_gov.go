...
func (a Service) RefuseOwnGov(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		a.log.WithError(err).Error("failed to refuse own citymod")
		switch {
		case errors.Is(err, errx.ErrorCannotRefuseMayor):
			ape.RenderErr(w, problems.Forbidden("not enough rights to refuse own citymod"))
	...
	a.log.Infof("user %s refused own citymod successfully", initiator.ID)
